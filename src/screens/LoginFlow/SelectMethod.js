...
const SelectMethod = () => {
  ...
  const {plan, selectedHours, totalAmount, paymentType = 'subscription'} = route.params || {};
  ...
  const preparePaymentData = useCallback(() => {
    ...
    console.log('Preparing payment data with:', {
      plan,
      totalAmount,
      selectedHours,
      plan_id: plan?.id,
    ...
    const paymentAmount = totalAmount || plan?.cost || plan?.price;
    ...
    return {
      ...
      hours: plan?.id === 2 ? (selectedHours || 0) : 0, // Only for Plan ID 2
    };
  ...
  const handleContinue = useCallback(async () => {
    ...
    try {
      ...
      if (selectedPaymentMethod === 'paypal') {
        ...
        if (result.success) {
          navigation.navigate('PayPalWebView', {
            ...
            planData: {
              ...plan,
              hours: plan?.id === 2 ? (selectedHours || 0) : 0,
            },
            planDetails: {
              name: plan?.plan_title || plan?.name || 'Plan',
        ...
        console.log('Stripe initialization result:', paymentIntentPayload);


        //  const stripePaymentIntentResult = await PaymentService.confrimPaymentIntentStripe(
        //   paymentIntentPayload,
        //   dispatch,
        // );

      // console.log('Stripe stripePaymentIntentResult result:', stripePaymentIntentResult);
      ...
      if (error.message.includes('Plan ID is required')) {
        showMessage({
          message: 'Plan Selection Error',
          description: 'Please go back and select a valid plan.',
          type: 'danger',
        });
      } else if (error.message.includes('payment amount')) {
        showMessage({
        ...
        {plan && (
          <View style={styles.planSummary}>
            <Text style={styles.planSummaryTitle}>
              {plan.plan_title || plan.name}
            </Text>
            <Text style={styles.planSummaryAmount}>
              {plan.currency?.toUpperCase() || 'AUD'} ${totalAmount || plan.cost || plan.price}
            </Text>
            {plan?.id === 2 && selectedHours > 0 && (
              <Text style={styles.planSummaryHours}>{selectedHours} hours</Text>
            )}
            {plan.no_of_days && (
              <Text style={styles.planSummaryDuration}>
                {plan.no_of_days} days access
              </Text>
            )}
          </View>
        )}
