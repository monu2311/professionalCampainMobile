...
const PaymentForm = ({ paymentData, planDetails, onSuccess, onError, initialClientSecret, initialPaymentIntentId, paymentType = 'subscription' }) => {
  ...
  useEffect(() => {
    const loadStoredUserData = async () => {
      try {
        ...
        console.log('Loaded stored user data for payment:', stored);
  ...
  const resolveUserData = () => {
    const fallbackData = {
      user_id: profileData?.user_id || userData?.id || storedUserData?.user_id || storedUserData?.details?.id || paymentData.userId,
    ...
    console.log('Resolved user data for payment:', fallbackData);
    console.log('Stored user data details:', storedUserData?.details);
    return fallbackData;
  };

  const resolvedUserData = resolveUserData();
  const membershipStatus = resolvedUserData.has_active_membership;
  ...
  console.log("Is Companion is true or false", resolvedUserData?.profile_type === 1 || resolvedUserData?.profile_type === '1'
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // Prepare billing details using resolved user data
      const billingResolvedData = resolveUserData();
      const billingDetails = {
        email: billingResolvedData?.email || paymentData.email || 'customer@example.com',
        name: billingResolvedData?.user_name || paymentData.name || 'Customer',
        phone: billingResolvedData?.phone_no || null,
        address: {
          country: billingResolvedData?.country || null,
          state: billingResolvedData?.state || null,
        }
      };
      ...
      console.log('Stripe payment confirmed successfully', paymentIntentId);

      // Use resolved user data for all payment confirmation logic
      const confirmationResolvedData = resolveUserData();
      const currentMembershipStatus = confirmationResolvedData.has_active_membership;
      const userIsUserCanLoggedIn = confirmationResolvedData.is_user_can_logged_in || storedUserData?.is_user_can_logged_in;

      // Determine plan_type based on profile_type (1 = management, else membership)
      const planType = (parseInt(confirmationResolvedData?.profile_type) === 1)
        ? currentMembershipStatus ? "management" : "companion"
        : "membership";

      // Validate required fields before confirmation
      if (!confirmationResolvedData.user_id) {
        throw new Error('User ID is required for payment confirmation. Please try logging in again.');
      }

      // Confirm payment on backend
      const confirmData = {
        payment_intent_id: paymentIntentId,
        user_id: confirmationResolvedData.user_id,
        plan_id: paymentData.plan_id,
        hours: paymentData.hours || paymentData.no_of_days || 0,
        plan_type: planType
      };

      console.log('Confirming payment with backend...', confirmData);
      ...
      if (backendResponse.success) {
        ...
        const profileType = confirmationResolvedData.profile_type?.toString();
        ...
        // Dynamic navigation based on profile type and previous membership status
        setTimeout(() => {
          if (profileType === '2') {
            // Members
            if (userIsUserCanLoggedIn === 'Expired') {
              // Member with expired membership goes to Home
              navigation.reset({
                index: 0,
                routes: [{ name: 'Home' }],
              });
            } else if (userIsUserCanLoggedIn === 'Not Purchased' || !userIsUserCanLoggedIn) {
            ...
            // Companions go to Home after payment
            navigation.reset({
              index: 0,
              routes: [{ name: 'Home' }],
            });
