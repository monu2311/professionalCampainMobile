...
import { useForgetPasswordValidation } from '../../hooks/useValidation';
...
const ForgetPassword = () => {
  ...
  const { schema: validationSchema } = useForgetPasswordValidation();
  ...
  const handleResetSubmit = async (values) => {
    if (!values.email || values.email.trim() === '') {
      showSnackbar(t('forms:validation.required'), 'error');
      return;
    }

    showSnackbar(t('forms:auth.sending'), 'info');
    const result = await performPasswordReset(values);

    if (result.success) {
      showSnackbar(t('screens:auth.resetPassword') + ' instructions sent!', 'success');
    }
              ...
              <Formik
                ...
                {({ handleSubmit, values, touched, setFieldValue, setFieldTouched }) => (
                  <View style={styles.formContainer}>
                    <Text style={styles.resetTitle}>{t('screens:auth.forgetPassword')}</Text>
                    <Text style={styles.resetSubtitle}>
                      {t('screens:auth.forgetPasswordInstructions')}
                    </Text>
                    ...
                    {/* Reset Button */}
                    <ButtonWrapper
                      label={isLoading ? t('forms:auth.sending') : t('forms:auth.send')}
                    ...
                    <View style={styles.loginContainer}>
                      <Text style={styles.loginText}>
                        {t('screens:auth.rememberPassword')}{' '}
                        <Text
                          style={styles.loginLink}
                          onPress={navigateToLogin}
                          disabled={isLoading}>
                          {t('screens:auth.signIn')}
                        </Text>
                      </Text>
                    </View>
