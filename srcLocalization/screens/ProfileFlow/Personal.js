...
const Personal = () => {
  ...
  const { t } = useTranslation([NAMESPACES.FORMS, NAMESPACES.COMMON]);
  const rtl = useRTL();
      ...
      <Formik
        ...
        {({handleSubmit, values, handleChange, setFieldValue}) => (
          <View
            ...
            <CustomTextInput
              label={t('forms:profile.workingName')}
              placeholder={t('forms:profile.workingNamePlaceholder')}
              name={'name'}
              // inputContainer={{
            ...
            <Select
              label={t('forms:auth.gender')}
              placeholder={t('forms:auth.genderPlaceholder')}
              name={'gender'}
              data={genderList}
              containerStyle={{width: '92%'}}
            />
            <Select
              label={t('forms:auth.profileType')}
              placeholder={t('forms:auth.profileTypePlaceholder')}
              name={'profile_type'}
            ...
            <Select
              ...
              placeholder={t('forms:location.selectCityPlaceholder')}
              name={'state_id'}
              data={
